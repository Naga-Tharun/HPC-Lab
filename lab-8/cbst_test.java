...
public class cbst_test{
	...
	public cbst_test(int num_threads, int range, int time, int pre_fill, int s_Limit, int i_Limit){
		instance = new cbst();
		THREADS = num_threads;
		RANGE = range;
		TIME = time;
	...
	class Fill extends Thread{		 
		...
		public void run(){
			...
			for(int i=0; i<PER_THREAD_PREFILL;){
				...
				boolean ins = instance.insert(val1, "value");
				if(ins){ 
					i = i+1;  
				}
			}
		...
		public void run(){	
			...
			for(int i=0; (end-start)<=TIME; i=i+1){
				...
				switch(ch){
					...
					case 1:{
						boolean ins = instance.insert(val, "value");
					}break;
					case 2:{
