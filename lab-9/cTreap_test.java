...
public class cTreap_test{
	...
	public cTreap_test(int num_threads, int range, int time, int pre_fill, int s_Limit, int i_Limit){
		instance = new cTreap();
		THREADS = num_threads;
		RANGE = range;
		TIME = time;
	...
	class Fill extends Thread{		 
		...
		public void run(){
			...
			for(int i=0; i<PER_THREAD_PREFILL;){
                ...
                int pri = (int)(Math.random()*(10000-10+1)+10);
				boolean ins = instance.insert(val1, pri);
				if(ins){ 
					i = i+1;  
				}
			}
		...
		public void run(){	
			...
			for(int i=0; (end-start)<=TIME; i=i+1){
				...
				switch(ch){
					...
					case 1:{
                        int pri = (int)(Math.random()*(10000-10+1)+10);
						boolean ins = instance.insert(val, pri);
					}break;
					case 2:{
						boolean removed = instance.delete(val); 
					}break;
					default: break;
				}
				count = count+1; 
	...
	public static void main(String[] args){
		...
		cTreap_test ob = new cTreap_test(num_threads,range,time,pre_fill,s_Limit,i_Limit);
